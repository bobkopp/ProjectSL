...
% expand grid in Alaska

alaska=shaperead('usastatehi','Selector',{@(name) strcmpi(name,'Alaska'),'Name'});
sub=find(alaska.X<-145);
sitecoords=[alaska.Y(sub)' alaska.X(sub)'];
sub=1:length(sub);
mx1=min(sitecoords(sub,2));
mx2=max(sitecoords(sub,2));
my1=min(sitecoords(sub,1));
...
clear Dx1 Dx2 Dy1 Dy2;
mdist=min(dist,[],2);
subdo=find(mdist<distcutoff/2);

GXa=[GX(subdo)];
GYa=[GY(subdo)];
siteids2=[siteids2 ; 1e9+1e4*[round((90-GY(subdo))*10)]+round(mod(GX(subdo),360)*10)];
for lll=1:length(subdo)
    sitenames2{end+1}=sprintf('grid_%0.1f_%0.1f',[GY(subdo(lll)) mod(GX(subdo(lll)),360)]);
end
