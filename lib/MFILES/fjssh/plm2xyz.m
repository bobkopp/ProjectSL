...
% plm2xyz('demo6') % EGM2008 topography
...
%
% Last modified by fjsimons-at-alum.mit.edu, 05/07/2009
...
if ~isstr(lmcosi)
  ...
  elseif length(degres)==1 && length(c11cmn)==4
    % It's a grid
    if degres>degN
      warning('PLM2XYZ: You can do better! Ask for more spatial resolution')
    ...
    disp(sprintf('Creating %i by %i grid with resolution %8.3f',nlon,nlat,degres))
  else
  ...
  for ldeg=1:size(els,1)
	...
	in2=in1+l+2;
	if [lup-ldown]>taskmax || length(degres) > 9999
	  if feature('ShowFigureWindows'), 	waitbar((l-ldown+1)/(lup-ldown+1),h), end;
	end
      end
      if [lup-ldown]>taskmax || length(degres) > 9999
	 if feature('ShowFigureWindows'), delete(h), end;
      end
...
elseif strcmp(lmcosi,'demo7')
  ...
  [a1,a2,a3,a4,a5,a6,a7,a8,a9,ronm]=addmon(L);
